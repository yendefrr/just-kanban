...
<template>
  <div class="flex w-full justify-start min-h-screen overflow-scroll">
    <div v-for="status in columnStatuses" class="flex" @dragover.prevent
      @drop="props.modelValue ? drop($event, status) : null">
      <KanbanColumn :modelValue="props.modelValue" :key="status" :status="status"
        :cards="cards.filter((card) => card.status === status)" @addCard="addCard" @moveCard="moveCard"
        @editStatus="editStatus"></KanbanColumn>
    </div>
  ...
  <Dialog v-model:visible="visibleDialog" modal header="New column" :style="{ width: '25rem' }">
    ...
    <div class="flex">
      <Button class="w-full" type="button" label="Save" @click="addColumn"
