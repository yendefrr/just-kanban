...
<template>
  <div
    class="min-w-[20rem] h-[45rem] bg-white shadow-lg rounded-[10px] p-4 flex-1 mx-2"
    :class="{ 'border-dashed': modelValue, 'border-[3px]': modelValue, 'opacity-40': onDrag, 'opacity-60': onDragOver }"
    :draggable="modelValue"
    @drop="drop($event)"
    @dragstart="modelValue ? dragStart($event) : null"
    @dragenter="modelValue ? (onDragOver = true) : null"
    @dragleave="modelValue ? (onDragOver = false) : null"
  >
    <div class="flex justify-between items-center mb-3 gap-3">
      ...
      <div class="flex gap-2">
        <Button v-if="!isEditColumn" class="h-[40px]" label="Edit" @click="isEditColumn = true" severity="info" outlined />
  ...
  <Dialog v-model:visible="visibleDialog" header="New card" :style="{ width: '25rem' }">
    ...
    <div class="flex">
      <Button class="w-full" type="button" label="Save" @click="addCard" v-bind:disabled="dialogData.title.length < 1"></Button>
    </div>
  </Dialog>
</template>
